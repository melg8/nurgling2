...
public class MCache implements MapSource {
    ...
    public static final Coord2d tilehqsz = Coord2d.of(1.375, 1.375);
    ...
    public Indir<Resource>[] sets = new Indir[16];
    @SuppressWarnings("unchecked")
	...
	public String tilesetname(int i) {
		Indir<Resource>[] sets = this.sets;
		if(i >= sets.length || sets[i] == null)
			return(null);
		if(sets[i] instanceof Resource.Spec)
			return(((Resource.Spec)sets[i]).name);
		try {
			return(sets[i].get().name);
		} catch(Loading e) {
			return(null);
		}
	}
