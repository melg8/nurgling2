...
- [x] 6.1 Протестировать переход через cave (поверхность → подземелье)
- [x] 6.2 Проверить создание маркеров "Cave XXX IN" и "Cave XXX OUT"
- [x] 6.3 Протестировать обратный переход — проверить дедупликацию
- [x] 6.4 Протестировать переход через minehole (уровень N → N+1)
- [x] 6.5 Протестировать переход через ladder (уровень N+1 → N)
- [x] 6.6 Проверить создание маркеров "Hole XXX IN" и "Hole XXX OUT"
- [x] 6.7 **Протестировать работу при отключённом ChunkNav overlay**
- [x] 6.8 Протестировать массовую разметку — пройти через одну cave, проверить разметку всех cave на сегменте
- [x] 6.9 Проверить логирование — наличие записей в logs/portal_transitions.log, logs/marker_events.log
- [x] 6.10 Проверить исключение teleports (hearthfire) — маркеры не создаются

## 7. Документация и полировка

- [x] 7.1 Добавить JavaDoc для всех публичных классов и методов
- [x] 7.2 Добавить обработку ошибок и graceful degradation
- [x] 7.3 Проверить код на соответствие code style проекта
